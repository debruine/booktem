...
The goal of booktem is to make the creation of methods books quick and easy. It uses a custom version of [webexercises](https://psyteachr.github.io/webexercises/) built to work with quarto (I'll update that package to match soon).
...
Books are set up with some lightweight glossary functions. Edit the file `include/glossary.yml` with your glossary terms like this:
...
Look up a term from the glossary file with `glossary("alpha")`: `r glossary("alpha", link = FALSE)`

Display a different value for the term with `glossary("alpha", "$\\alpha$")`: `r glossary("alpha", "$\\alpha$", link = FALSE)`

Use an inline definition instead of the glossary file with `glossary("beta", def = "The second letter of the Greek alphabet")`: `r glossary("beta", def = "The second letter of the Greek alphabet", link = FALSE)`

Just show the definition with `glossary("p-value", show_def = TRUE)`: `r glossary("p-value", show_def = TRUE, link = FALSE)`
